...
const { User, DiveShop, Certification, OfferedDive , Log, DivesOfferedByShops} = require('../server/db/models')
...
async function seed() {
  ...
  const users = await Promise.all([
    User.create({email: 'cody@email.com', password: '123', firstName
    : 'Cody', lastName: 'De Coder'}),
        User.create({email: 'murphy@email.com', password: '123', firstName
    : 'Murphy', lastName: 'Law'})
  ])
  ...
  const offereddive = await Promise.all([
    ...
    OfferedDive.create({
      ...
      description: 'The Great Blue Hole is a giant marine sinkhole off the coast of Belize. It lies near the center of Lighthouse Reef.'
    }),
  ])
